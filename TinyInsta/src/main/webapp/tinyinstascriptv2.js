...
app.controller('TController',['$scope','$window', function($scope,$window) {
    ...
    $scope.listtwitt = [{author: 'admin',message :'Hello and Welcome to Tiny Insta'}];
    $scope.author = '';
    $scope.login ='';
    $scope.message;
    $scope.log = false;
    ...
    $scope.timeline = function(nom){
        ...
        gapi.client.tinyinstaAPI.getTimeline({
            login: nom
        }).execute(function(resp){
            console.log(resp);
            if(resp.code != 503){
    ...
    $scope.follow = function(nom){
        if($scope.log == true){
            gapi.client.tinyinstaAPI.followUser({
    ...
    $scope.connexion = function(nom){
        ...
        gapi.client.tinyinstaAPI.getUser({
            login: nom
        }).execute(function(resp){
    ...
    $scope.postTwitt = function(){
        if($scope.log == true){
            gapi.client.tinyinstaAPI.createTwitt({
                login: $scope.author,
    ...
    $scope.twittnbfollowers = function(pseudon,nbfollowerstest){
                    ...
                    gapi.client.tinyinstaAPI.createNbFollowers({
   ...
   /* $scope.twittgetlastten = function(pseudal,nbfollowerstestten) {
    	...
    	gapi.client.tinyinstaAPI.getLastTens({
    		login: pseudal
    	}).execute(function(resp){
            console.log(resp);
            if(resp.code != 503){
    ...
    $window.init = function() {
          ...
          var rootApi = 'https://tinyinsta-259813.appspot.com/_ah/api/';
